...
const initialize = () =>{
	document.getElementById("title").style.color = randomValue;
	document.getElementById("linkChange").style.color = randomValue;
	document.getElementById("linkChange-2").style.color = randomValue;
	document.getElementById("linkChange-3").style.color = randomValue;
};
...
function canvasApp() {
	...
	function init() {
		...
		*/
		numCircles = randomCircles;
		maxMaxRad = randomSize;
		minMaxRad = randomSize;
	...
	function setCircles() {
		...
		for (i = 0; i < numCircles; i++) {
			...
			grad.addColorStop(0,"rgba(60,86,107,0.3)");
			grad.addColorStop(1,"rgba(214,182,141,0.2)");
	...
	function onTimer() {
		...
		for (j = 0; j < drawsPerFrame; j++) {
			...
			for (i = 0; i < numCircles; i++) {
				...
				c.centerX += 0.5;
				c.centerY += 0.04;
				yOffset = 60*Math.sin(c.globalPhase + drawCount/2000*TWO_PI);
